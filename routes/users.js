...
router.get('/getmycourses',injectUerforAPI, async function (req, res, next){
	...
	try {
		...
		res.status(200).send({
			result: true,
			data: rows,
			message: `해당하는 사용자의 수강한 강의 정보를 출력합니다.`
		})
	} catch (error) {
		console.log(error)			
	}
...
router.get('/getcontainer',injectUerforAPI, async function (req, res, next){
	...
	try {
		...
		}else{ //! 디비에 없는데 현재 Container가 있는걸 처리해야 될까요?
			res.status(201).send({
				result: true,
				data: [],
				message: `해당하는 사용자의 컨테이너 정보 없음.`
	...
	try {
		...
		if(row.length === 0)
		{
			...
			if(!isExits){
				...
				console.log("실래 컨테이너 존재함")
				res.status(400).send({
					result: false,
					data: [],
					message: `컨테이너 생성 실페 이미 존재함`
				})
			}
		}else{
			res.status(201).send({
